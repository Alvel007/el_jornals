...
class DispatchCompanies(models.Model):
    ...
    abbreviated_name = models.CharField(max_length=NAME_MAX_LENGTH,
                                        verbose_name='Сокращенное наименование',
                                        unique=True, blank=False,)
    abbreviated_name_rp = models.CharField(max_length=NAME_MAX_LENGTH,
                                           verbose_name='',
                                           unique=True, blank=False,
...
class ThirdPartyDispatchers(models.Model):
    disp_post = models.CharField(max_length=NAME_MAX_LENGTH,
                                 verbose_name='Должность персонала стороннего ДЦ',
                                 null=True, blank=False,)
    disp_name = models.CharField(max_length=NAME_MAX_LENGTH,
                                 unique=True,
                                 verbose_name='Фамилия И.О.',
                                 null=True, blank=False,)
    disp_center = models.ForeignKey(DispatchCompanies,
    ...
    def clean(self):
        ...
        existing_dispatchers = ThirdPartyDispatchers.objects.filter(disp_name=self.disp_name)
        if self.pk:
            existing_dispatchers = existing_dispatchers.exclude(pk=self.pk)
        if existing_dispatchers.exists():
            raise ValidationError('Диспетчер с такими ФИО уже существует. Укажите инициалы, состояние из 2ух букв!')
...
class PowerLine(models.Model):
    ...
    disp_manage = models.ForeignKey(DispatchCompanies,
                                 on_delete=models.PROTECT,
                                 null=True, blank=True,
    ...
    slug = models.SlugField(verbose_name='Слаг',
                            unique=True,
                            editable=False)
    ending = models.ManyToManyField(Substation,
                                    blank=True,
                                    verbose_name='ВЛ заходит на следующие ПС',
                                    limit_choices_to={'dispatch_point': False},
                                    help_text='Обязательное поле',
                                    related_name='powerline_endings')
    third_party_object_1 = models.CharField(max_length=NAME_MAX_LENGTH,
                            verbose_name='Сторонний объект 1',
                            null=True, blank=True,
                            help_text='Если ВЛ подключена к объектам третьих лиц, укажите ДН этого объекта (необязательное поле). Объекты генерации указывать в родительском падеже.')
    third_party_object_2 = models.CharField(max_length=NAME_MAX_LENGTH,
                            verbose_name='Сторонний объект 2',
                            null=True, blank=True,
                            help_text='Если ВЛ подключена к объектам третьих лиц, укажите ДН этого объекта (необязательное поле). Объекты генерации указывать в родительском падеже.')
    third_party_object_3 = models.CharField(max_length=NAME_MAX_LENGTH,
    ...
    induced_voltage = models.BooleanField(verbose_name='Наличие наведенного напряжения на ВЛ',
                                          default=False,
                                          null=False, blank=False)
    for_CUS_dispatcher = models.ManyToManyField(Substation,
                                 blank=True,
                                 verbose_name='Интересна диспетчеру',
                                 limit_choices_to={'dispatch_point': True},
                                 help_text='В эксплуатационной ответственности какого диспетчера находится указанная ВЛ?',
...
class AdmittingStaff(models.Model):
    ...
    organization = models.CharField(max_length=NAME_MAX_LENGTH,
                                    verbose_name='От организации',
                                    null=True, blank=True,
                                    help_text='Организация, где работает допускающий. В родительном падеже (нет чего?)')
    for_CUS_dispatcher = models.ManyToManyField(Substation,
                                 blank=True,
                                 verbose_name='Для диспетчера ЦУС',
                                 limit_choices_to={'dispatch_point': True},
                                 help_text='Какому диспетчеру будет доступен этот допускающий для заполнения автоформы',
    ...
    def clean(self):
        ...
        if existing_admitting.exists():
            raise ValidationError('Допускающий с такими ФИО уже существует. Укажите инициалы, состояние из 2ух букв!')
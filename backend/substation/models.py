...
class Substation(models.Model):
    ...
    address = models.CharField(
        max_length=128,
        verbose_name='Адрес',)
    group_substation = models.ForeignKey(
        'GroupSubstation',
    ...
    slug = models.SlugField(
        ...
        unique=True,
        editable=False,)
    dispatch_point = models.BooleanField(
        verbose_name='Является диспетчерским пунктом',
        default=False)
    dispatcher_for = models.ManyToManyField(
        'self',
        blank=True,
...
class GroupSubstation(models.Model):
    ...
    name_rp = models.CharField(
        max_length=NAME_MAX_LENGTH,
        verbose_name='Название СП в родительском падеже',
        null=True,
        blank=True,
        help_text=('Название группы этого же структурного '
                   'подразделения в родительском падеже'),)
    slug = models.SlugField(
        verbose_name='Слаг',
        unique=True,
        editable=False,)

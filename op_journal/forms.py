...
class MainPageOPJournalForm(forms.ModelForm): 
    ...
    important_event_checkbox = forms.BooleanField(required=False, label='Вывод из работы/отключение')
    permission_to_work_checkbox = forms.BooleanField(required=False, label='Допуск к выполнению работ')
    ...
    text = forms.CharField(widget=forms.Textarea(attrs={'id': 'autocomplete-text', 'wrap': 'soft'}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['planned_completion_date'].initial = timezone.localtime(timezone.now()).strftime('%Y-%m-%dT%H:%M')
        ...
        if substation_slug:
            ...
            self.fields['text'].widget = forms.Textarea(attrs={
                'autocomplete': 'off',
                'data-autocomplete-url': reverse('substation_autocomplete', args=[substation_slug]) if substation_slug else '',
                'wrap': 'soft',  # для переноса на новую строку при достижении конца строки
                'onkeydown': "if(event.key=='Enter'){this.value+='\n';event.preventDefault();}"  # обработка нажатия Enter
            })
    ...
    def clean_planned_completion_date(self):
        important_event_checkbox = self.cleaned_data.get('important_event_checkbox')
        planned_completion_date = self.cleaned_data.get('planned_completion_date')

        if important_event_checkbox:
            if planned_completion_date and planned_completion_date <= timezone.now():
                raise forms.ValidationError('Планируемая дата ввода в работу оборудования должна быть в будущем.')
    ...
    def save(self, commit=True):
        ...
        withdrawal_for_repair = self.cleaned_data.get('withdrawal_for_repair')
        planned_completion_date = self.cleaned_data.get('planned_completion_date')
    ...
    class Meta: 
        ...
        fields = ['text', 'pub_date', 'substation', 'special_regime_introduced', 'emergency_event', 'short_circuit', 'file', 'important_event_checkbox', 'permission_to_work_checkbox', 'existing_entry', 'work_entry', 'planned_completion_date']
...
class OPJournalForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not planned_completion_date:
            cleaned_data['planned_completion_date'] = self.instance.planned_completion_date
        return cleaned_data

    def clean_planned_completion_date(self):
        planned_completion_date = self.cleaned_data.get('planned_completion_date')
        initial_planned_completion_date = self.instance.planned_completion_date if self.instance.pk else None
        withdrawal_for_repair = self.instance.withdrawal_for_repair if self.instance.pk else False
        closing_entry = self.instance.closing_entry if self.instance.pk else None

        if withdrawal_for_repair and not closing_entry: 
            if planned_completion_date and initial_planned_completion_date and planned_completion_date < initial_planned_completion_date:
                raise forms.ValidationError("Дата продления не может быть меньше, указанной ранее, при выводе оборудования из работы!")

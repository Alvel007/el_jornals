...
class MainPageOPJournalForm(forms.ModelForm):
    pub_date = forms.DateTimeField(initial=timezone.now, input_formats=['%d.%m.%Y %H:%M:%S'])
    ...
    def get_absolute_url(self):
        return reverse('op_journal', kwargs={'substation_slug': self.instance.substation.slug})


class OPJournalForm(forms.ModelForm):
    class Meta:
        model = MainPageOPJournal
        fields = ['entry_is_valid']  # Указываем только поле entry_is_valid для отображения на странице

        widgets = {
            'entry_is_valid': forms.CheckboxInput(attrs={'class': 'form-check-input'})  # Используем виджет CheckboxInput для поля entry_is_valid
        }